const Shape = React.createClass({
  ...
  render () {
    ...
    let name = this.state.editable ? <input type='text' ref='name' defaultValue={shape.name} /> : <h3>Shape: {shape.name}</h3>
    let color = this.state.editable ? <input type='text' ref='color' defaultValue={shape.color} /> : <p>Color: {shape.color}</p>;
